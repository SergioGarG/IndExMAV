...
void CallbackLecturaKalman(const ekf::mensaje_kalman_array::ConstPtr& msg)
{
	...
	pos_kalman[3]=msg->estado_vector[8].estado;
}

int main(int argc, char **argv)
{
	ros::init(argc, argv, "pid_main");
	ros::NodeHandle n;

	pid_objeto pid_obj;

	//Suscriptores
	ros::Subscriber sub_poseEKF = n.subscribe("kalman_topic", 1000, CallbackLecturaKalman); //El nodo al que se suscribe para leer la salida del f de kalman

	//Publicadores
	ros::Publisher chatter_pub = n.advertise<geometry_msgs::Twist>("bebop/cmd_vel", 1000);
	ros::Publisher pub_errores = n.advertise<pid::pid_errores>("pid_errores", 1000);
